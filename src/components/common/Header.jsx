...
import { 
  ...
  Layers, Tag, Award, LogOut, LayoutDashboard, Truck, ShieldCheck, PhoneCall
} from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto flex flex-wrap justify-between items-center gap-2">
          <div className="flex items-center gap-6">
            <span className="flex items-center gap-1.5 text-amber-400 font-medium">
              ...
              <span>توصيل سريع وفوري لجميع مراكز محافظة بني سويف 🇪🇬</span>
            </span>
            <span className="hidden md:flex items-center gap-1 text-gray-300">
              <ShieldCheck className="w-3.5 h-3.5 text-green-400" />
              <span>منتجات أصلية 100% بأفضل سعر</span>
            </span>
          </div>
          ...
          <div ref={searchRef} className="flex-1 max-w-2xl relative hidden md:block">
            ...
            {isSearchOpen && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-2xl border border-gray-100 p-3 z-50 max-h-[420px] overflow-y-auto">
                {/* Related Brands suggestions */}
                {relatedBrands.length > 0 && (
                  <div className="mb-3 pb-2 border-b border-gray-100">
                    <span className="text-[11px] font-bold text-gray-400 block mb-1.5">ماركات وبراندات مقترحة:</span>
                    <div className="flex flex-wrap gap-2">
                      {relatedBrands.map((b) => (
                        <Link
                          ...
                          className="flex items-center gap-1.5 px-2.5 py-1 bg-gray-50 hover:bg-red-50 text-gray-700 hover:text-brand-red rounded-lg text-xs font-bold border border-gray-200 transition"
                        >
                          <span>{b.name}</span>
                          <span className="text-[10px] text-gray-400">({b.productCount || 'منتجات'})</span>
                        </Link>
                      ))}
                    </div>
                  </div>
                ...
                <div className="text-xs font-bold text-gray-400 px-3 py-1 mb-1">
                  نتائج المنتجات ({searchResults.length})
                </div>
                {searchResults.length > 0 ? (
                  ...
                  <div className="text-center py-6 text-gray-500">
                    ...
                    <p className="text-xs text-gray-400 mt-1">جرب كلمات أخرى مثل: زبادي، حليب، أريال، زيت...</p>
                  </div>
                )}
              </div>
            )}
